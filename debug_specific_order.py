#!/usr/bin/env python3
"""
Debug the specific order that's not sending email
"""
import sys
import os
...
def debug_specific_order():
    """
    Debug the specific order ID that's not working
    """
    print("=" * 80)
    print("DEBUGGING SPECIFIC ORDER: ORD-1772098137784")
    print("=" * 80)
    ...
    try:
        ...
        if email_sent_at:
            print(f"⚠️  Email already sent at: {email_sent_at}")
            print("This explains why you're not getting another email")
        else:
            print("✅ Email not sent yet - this is expected")
        ...
        print(f"\n" + "=" * 60)
        print("SENDING CONFIRMATION EMAIL FOR THIS ORDER")
        print("=" * 60)
        ...
        user_name = "Customer"
        created = order.get("created") or order.get("updated_at") or datetime.now()
