...
1. **Upload prescription on product page:**
   ```bash
   curl -X POST https://testbackend.multifolks.com/api/v1/prescriptions/upload-image \
...
2. **Add product to cart:**
   ```bash
   curl -X POST https://testbackend.multifolks.com/api/v1/cart/add \
     -H "Authorization: Bearer YOUR_TOKEN" \
...
3. **Check cart:**
   ```bash
   curl https://testbackend.multifolks.com/api/v1/cart \
     -H "Authorization: Bearer YOUR_TOKEN"
   ```
